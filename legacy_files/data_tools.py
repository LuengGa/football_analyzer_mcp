...
from .output_schemas import AnalysisOutput, FetchTodayMatchesOutput
from lottery_mcp.analysis.constants import EMPIRICAL_RETURN_RATES, RETURN_RATE_TIERS
...
def _format_match_data_markdown(data: Dict) -> str:
    """将比赛数据格式化为Markdown"""
    lines = ["# 比赛详情", ""]
    ...
    if odds:
        ...
        had = odds.get("had", {})
        if had:
            lines.append(f"- 主胜: {had.get('win', '-')} | 平局: {had.get('draw', '-')} | 客胜: {had.get('lose', '-')}")
        lines.append("")
...
def _format_odds_changes_markdown(match_id: str, data: Dict) -> str:
    """将赔率变化数据格式化为Markdown"""
    lines = [f"# 赔率变化 - {match_id}", ""]

    if not data:
        lines.append("暂无赔率变化数据")
        return "\n".join(lines)

    # 显示各玩法赔率变化
    for play_type, odds_data in data.items():
        if isinstance(odds_data, dict):
            lines.append(f"## {play_type}")
...
def _format_verify_results_markdown(verification: Dict, sources: List[Dict]) -> str:
    """将开奖验证结果格式化为Markdown"""
    lines = ["# 开奖结果验证", ""]

    lines.append(f"**日期**: {verification.get('date', '-')}")
    lines.append(f"**彩种**: {verification.get('lottery_type', '-')}")
    lines.append(f"**验证状态**: {'✅ 已验证' if verification.get('is_verified') else '❌ 未通过'}")
    lines.append(f"**一致性**: {verification.get('consistency', '-')}")
    lines.append("")

    lines.append("## 数据源")
    for source in sources:
        status = source.get('status', '')
        status_icon = "✅" if status == "success" else "⚠️" if status == "supplementary" else "❌"
        lines.append(f"- {status_icon} **{source.get('source', '-')}**: {status}")
        if source.get('note'):
            lines.append(f"  - 备注: {source['note']}")
    lines.append("")
...
def _format_market_odds_markdown(matches: List[Dict], market_types: List[str]) -> str:
    """将市场赔率数据格式化为Markdown"""
    lines = ["# 市场赔率", ""]

    for match in matches:
        home = match.get("home_team_cn", match.get("home_team", ""))
        away = match.get("away_team_cn", match.get("away_team", ""))
        lines.append(f"## {home} vs {away}")

        if "european" in market_types and match.get("european_odds"):
            lines.append("### 欧赔")
            consensus = match.get("consensus", {})
            if consensus:
                lines.append(f"- 主胜: {consensus.get('home_win', '-')} | 平局: {consensus.get('draw', '-')} | 客胜: {consensus.get('away_win', '-')}")
            lines.append("")

        if "asian" in market_types and match.get("asian_handicap"):
            lines.append("### 亚盘")
            asian = match.get("asian_handicap", [])
            if asian:
                first = asian[0]
                lines.append(f"- 盘口: {first.get('handicap', '-')} | 主队: {first.get('home_odds', '-')} | 客队: {first.get('away_odds', '-')}")
            lines.append("")

        if "over_under" in market_types and match.get("over_under"):
            lines.append("### 大小球")
            ou = match.get("over_under", [])
            if ou:
                first = ou[0]
                lines.append(f"- 盘口: {first.get('line', '-')} | 大球: {first.get('over_odds', '-')} | 小球: {first.get('under_odds', '-')}")
            lines.append("")
...
def with_api_fallback(error_code: str = "DATA_FETCH_ERROR", fallback_msg: str = ""):
    """API调用统一降级装饰器
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                return _to_json({
                    "success": False,
                    "error": f"{fallback_msg or func.__name__}请求超时，请稍后重试",
                    "error_code": error_code,
                    "suggestion": "数据源响应缓慢，建议稍后重试或检查网络连接",
                })
            except Exception as e:
                logger.error(f"{func.__name__} 失败: {e}")
                return _to_json({
                    "success": False,
                    "error": f"{fallback_msg or func.__name__}获取失败: {e}",
                    "error_code": error_code,
...
def get_cached_matches() -> List[Dict[str, Any]]:
    ...
    with _match_cache_lock:
        ...
        return list(_match_cache)  # 返回副本防止外部修改
...
@with_api_fallback(error_code="DATA_FETCH_ERROR", fallback_msg="获取今日比赛")
...
async def lottery_get_match_data(params: GetMatchDataInput, ctx: Context) -> str:
    ...
    try:
        ...
        import asyncio
        tasks = []
        if params.data_type in ["full", "stats"] and params.league:
            tasks.append(("standings", manager.get_standings(params.league)))
        if params.data_type in ["full", "history"] and params.home_team and params.away_team:
            tasks.append(("h2h", manager.get_head_to_head(params.home_team, params.away_team)))
        ...
        if tasks:
            ...
            for (key, _), result in zip(tasks, results):
                if isinstance(result, Exception):
                    ...
                    match_data[key] = result["data"]
        
        await ctx.report_progress(1.0, "获取完成")

        # 根据 response_format 返回不同格式
        if params.response_format == "markdown":
            return _format_match_data_markdown(match_data)
...
def _generate_match_analysis(
    ...
    # 根据玩法类型获取返还率基准（SPF默认基准 ~88.6%）
    play_type_for_rr = "SPF"
    expected_rr = EMPIRICAL_RETURN_RATES.get(play_type_for_rr, 0.75)
    rr_deviation_threshold = 0.02  # 偏离基准2%视为异常

    if payout > expected_rr + rr_deviation_threshold:
        reasoning_chain["key_observations"].append(
            f"返还率偏高(>{expected_rr + rr_deviation_threshold:.0%})，赔率相对合理"
        )
        reasoning_chain["inference_steps"].append({
            "step": step_num,
            "input": "官方赔率返还率",
            "logic": f"返还率{payout:.2%}高于{play_type_for_rr}基准{expected_rr:.0%}+{rr_deviation_threshold:.0%}阈值",
            "output": "赔率定价相对公平，市场效率较高"
        })
        step_num += 1
    elif payout < expected_rr - rr_deviation_threshold:
        reasoning_chain["key_observations"].append(
            f"返还率偏低(<{expected_rr - rr_deviation_threshold:.0%})，存在抽水"
        )
        reasoning_chain["inference_steps"].append({
            "step": step_num,
            "input": "官方赔率返还率",
            "logic": f"返还率{payout:.2%}低于{play_type_for_rr}基准{expected_rr:.0%}-{rr_deviation_threshold:.0%}阈值",
...
async def lottery_get_live_odds(
    match_id: str,
    ...
    从实时数据客户端获取最新的赔率信息，支持缓存和强制刷新。

    Args:
        match_id: 比赛ID
        play_type: 玩法类型（SPF、RQSPF、BF、ZJQ、BQC），None则返回所有玩法
    ...
    try:
        if ctx:
            await ctx.log_info(f"[实时数据] 获取赔率: {match_id}, 玩法: {play_type or 'all'}")
        
        from lottery_mcp.data.realtime_client import get_live_odds
        
        result = await get_live_odds(match_id, play_type, force_refresh)
        
        # 格式化输出
        lines = [f"# 实时赔率 - {match_id}", ""]
        
        if result.get("error"):
            lines.append(f"⚠️ **警告**: {result['error']}")
            lines.append("")
        
        lines.append(f"- **数据源**: {result.get('source', 'unknown')}")
        lines.append(f"- **时间戳**: {result.get('timestamp', '-')}")
        lines.append(f"- **缓存状态**: {'已缓存' if result.get('cached') else '实时'}")
        lines.append("")
        ...
        if changes:
            lines.append("## 赔率变化")
            for change in changes[:5]:  # 只显示最近5条
                direction = "📈" if change.get("change_pct", 0) > 0 else "📉"
                lines.append(
                    f"- {direction} {change.get('play_type')} {change.get('selection')}: "
                    f"{change.get('old_odds')} → {change.get('new_odds')} "
                    f"({change.get('change_pct'):+.1f}%)"
                )
            lines.append("")
        ...
        logger.error(f"获取实时赔率失败: {e}")
        return f"获取实时赔率失败: {e}"


async def lottery_get_match_status(
    match_id: str,
    ctx: Optional[Context] = None,
) -> str:
    """获取比赛实时状态

    获取比赛的当前状态，包括是否开始、比分、进行时间等。
    ...
    try:
        if ctx:
            await ctx.log_info(f"[实时数据] 获取比赛状态: {match_id}")
        
        from lottery_mcp.data.realtime_client import get_match_status
        
        result = await get_match_status(match_id)
        
        # 格式化输出
        lines = [f"# 比赛状态 - {match_id}", ""]
        ...
        if minute is not None:
            lines.append(f"- **进行时间**: {minute} 分钟")
        
        match_time = result.get("match_time")
        if match_time:
            lines.append(f"- **比赛时间**: {match_time}")
        
        lines.append(f"- **更新时间**: {result.get('last_update', '-')}")
        lines.append(f"- **缓存状态**: {'已缓存' if result.get('cached') else '实时'}")
...
async def lottery_get_odds_changes(
    match_id: Optional[str] = None,
    min_significance: str = "significant",
    ctx: Optional[Context] = None,
) -> str:
    """获取显著赔率变化
    ...
    Args:
        match_id: 比赛ID，None则返回所有比赛的显著变化
        min_significance: 最小重要性级别 (normal, significant, major)
        ctx: MCP Context
    ...
    try:
        if ctx:
            await ctx.log_info(f"[实时数据] 获取赔率变化: {match_id or 'all'}, 级别: {min_significance}")
        
        from lottery_mcp.data.realtime_client import get_realtime_client
        
        client = get_realtime_client()
        changes = client.get_significant_changes(match_id, min_significance)
        
        # 格式化输出
        lines = ["# 显著赔率变化", ""]
        
        if not changes:
            lines.append("暂无显著赔率变化")
            return "\n".join(lines)
        
        lines.append(f"共发现 {len(changes)} 条显著变化:\n")
        
        for change in changes[:20]:  # 只显示最近20条
            direction = "📈" if change.get("change_pct", 0) > 0 else "📉"
            significance = change.get("significance", "normal")
            sig_emoji = {"major": "🔴", "significant": "🟡", "normal": "🟢"}.get(significance, "⚪")
            
            lines.append(
                f"{sig_emoji} {direction} **{change.get('match_id', 'unknown')}** "
                f"{change.get('play_type')} {change.get('selection')}: "
                f"{change.get('old_odds')} → {change.get('new_odds')} "
                f"({change.get('change_pct'):+.1f}%)"
            )
            lines.append(f"   时间: {change.get('timestamp', '-')}")
            lines.append("")
        
        return "\n".join(lines)
        
    except Exception as e:
        logger.error(f"获取赔率变化失败: {e}")
        return f"获取赔率变化失败: {e}"
...
典型工作流: fetch_today_matches → analyze_all_matches → generate_recommendation""",
        ...
        outputSchema=FetchTodayMatchesOutput,
    )
    ...
    @mcp.tool(
        name="lottery_query_match_data",
        description="""[数据] 查询比赛详细数据
获取指定比赛的各类数据：
- full: 完整数据
- odds: 仅赔率
- stats: 统计数据
...
典型工作流: 获取单场详细数据后，可调用 analyze_match 进行深度分析。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    async def _lottery_query_match_data(params: GetMatchDataInput, ctx: Context) -> str:
    ...
    @mcp.tool(
        name="lottery_monitor_odds_changes",
        description="""[监控] 监控竞彩赔率变化数据
获取竞彩足球指定场次的赔率变化信息，覆盖6种玩法：
- 胜平负(SPF)、让球胜平负(RQSPF)、比分(BF)
- 总进球(ZJQ)、半全场(BQC)、混合过关

典型工作流: 与 fetch_market_odds 配合使用，监控赔率变化趋势。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    async def _lottery_monitor_odds_changes(params: TrackOddsChangesInput, ctx: Context) -> str:
        try:
            ...
            # 根据 response_format 返回不同格式
            if params.response_format == "markdown":
                return _format_odds_changes_markdown(params.match_id, result)
...
典型工作流: 与 query_history 配合，先查询再验证。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    async def _lottery_verify_results(params: VerifyResultsInput, ctx: Context) -> str:
        try:
            ...
            # 根据 response_format 返回不同格式
            if params.response_format == "markdown":
                return _format_verify_results_markdown(verification, sources)
...
典型工作流: 查询历史数据后，可调用 analyze_results 进行统计分析。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        name="lottery_fetch_live_scores",
        description="""[数据] 获取实时比分
...
典型工作流: 获取实时数据后，可调用 monitor_odds_changes 和 assess_risk 进行实时分析。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @with_api_fallback(error_code="LIVE_SCORES_ERROR", fallback_msg="获取实时比分")
    async def _lottery_fetch_live_scores(params: GetLiveScoresInput, ctx: Context) -> str:
        try:
            manager = _get_manager()
    ...
    @mcp.tool(
        name="lottery_fetch_market_odds",
        description="""[数据] 获取市场赔率数据（统一入口）

典型工作流: 获取赔率后，可调用 find_value_bets 识别价值投注机会。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @with_api_fallback(error_code="MARKET_ODDS_ERROR", fallback_msg="获取市场赔率")
    async def _lottery_fetch_market_odds(params: GetMarketOddsInput, ctx: Context) -> str:
        try:
            manager = _get_manager()
            ...
            # 根据 response_format 返回不同格式
            if params.response_format == "markdown":
                return _format_market_odds_markdown(filtered, market_types)
...
async def lottery_get_live_odds(params: 'GetLiveOddsInput', ctx: Context) -> str:
    """获取滚球盘实时赔率数据
    ...
    """
    try:
        # 参数已经是GetLiveOddsInput类型（由MCP框架验证）
        league = params.league
...
def register_live_odds_tools(mcp: Any):
    ...
    try:
        ...
        def Field(default=..., **kwargs):
            pydantic_only_keys = {"description","examples","min_length","max_length","ge","gt","le","lt","pattern"}
            for key in list(kwargs.keys()):
                if key in pydantic_only_keys: kwargs.pop(key)
            if default is not ...: return dc_field(default=default, **kwargs)
    ...
    @mcp.tool(
        name="lottery_fetch_live_odds",
        description="获取实时赔率数据，支持欧赔、亚盘、大小球等多种市场类型",
        annotations={"readOnlyHint": True, "destructiveHint": False, "idempotentHint": True, "openWorldHint": True},
    )
    async def _lottery_fetch_live_odds(params: GetLiveOddsInput, ctx: Context) -> str:
...
def register_jingcai_info_tools(mcp: Any):
    ...
    @mcp.tool(
        name="lottery_fetch_match_info",
        description="""[数据] 获取竞彩比赛头部信息（对阵、时间、联赛等）。
典型工作流:
1. 传入 match_id 获取比赛基本信息
2. 返回比赛的基础元数据（ID、联赛、球队、时间、场地等）

参数说明:
- match_id: 竞彩比赛ID（如 '2025052510001'）""",
        annotations={"readOnlyHint": True, "idempotentHint": True, "destructiveHint": False, "openWorldHint": True},
       )
    async def _lottery_fetch_match_info(params: MatchIdInput, ctx: Context) -> str:
        return await _lottery_get_match_info(params.match_id, ctx)
    
    @mcp.tool(
        name="lottery_fetch_match_features",
        description="""[数据] 获取竞彩比赛特征分析（攻防特点、比赛风格等）。
典型工作流:
1. 传入 match_id 和 limit 获取比赛特征分析
2. 返回攻防统计、风格分析和交锋特点总结

参数说明:
- match_id: 竞彩比赛ID（如 '2025052510001'）
- limit: 返回数据条数限制（默认10条，范围1-50）""",
        annotations={"readOnlyHint": True, "idempotentHint": True, "destructiveHint": False, "openWorldHint": True},
       )
    async def _lottery_fetch_match_features(params: MatchIdWithLimitInput, ctx: Context) -> str:
        return await _lottery_get_match_features(params.match_id, params.limit, ctx)
    
    @mcp.tool(
        name="lottery_fetch_jingcai_h2h",
        description="""[数据] 获取竞彩历史交锋数据（双方对战记录、进球统计等）。
典型工作流:
1. 传入 match_id 和 limit 获取历史交锋数据
2. 返回双方直接对战记录、进球统计和胜负关系
...
- limit: 返回数据条数限制（默认10条，范围1-50）""",
    ...
    async def _lottery_fetch_jingcai_h2h(params: MatchIdWithLimitInput, ctx: Context) -> str:
        return await _lottery_get_jingcai_h2h(params.match_id, params.limit, ctx)
    
    @mcp.tool(
        name="lottery_fetch_match_standings",
        description="""[数据] 获取竞彩比赛相关的联赛积分榜（主队/客队排名及完整排名表）。
典型工作流:
1. 传入 match_id 获取相关联赛积分榜
2. 返回主队排名、客队排名以及完整联赛排名表

参数说明:
- match_id: 竞彩比赛ID（如 '2025052510001'）""",
        annotations={"readOnlyHint": True, "idempotentHint": True, "destructiveHint": False, "openWorldHint": True},
       )
    async def _lottery_fetch_match_standings(params: MatchIdInput, ctx: Context) -> str:
        return await _lottery_get_match_standings(params.match_id, ctx)
    
    @mcp.tool(
        name="lottery_fetch_recent_form",
        description="""[数据] 获取竞彩比赛两队的近期战绩（最近比赛记录及胜平负统计）。
典型工作流:
1. 传入 match_id 和 limit 获取两队近期战绩
2. 返回主队和客队的最近比赛记录及战绩统计（胜/平/负）
...
- limit: 返回数据条数限制（默认10条，范围1-50）""",
    ...
    async def _lottery_fetch_recent_form(params: MatchIdWithLimitInput, ctx: Context) -> str:
        return await _lottery_get_recent_form(params.match_id, params.limit, ctx)
    
    @mcp.tool(
        name="lottery_fetch_future_matches",
        description="""[数据] 获取竞彩比赛相关的未来赛事（主客队后续赛程预告）。
典型工作流:
1. 传入 match_id 和 limit 获取相关未来赛事
2. 返回主队和客队的下一轮比赛及关键赛事预告
...
- limit: 返回数据条数限制（默认10条，范围1-50）""",
    ...
    async def _lottery_fetch_future_matches(params: MatchIdWithLimitInput, ctx: Context) -> str:
        return await _lottery_get_future_matches(params.match_id, params.limit, ctx)
    
    @mcp.tool(
        name="lottery_fetch_players",
        description="""[数据] 获取竞彩比赛的关键球员信息（射手、近期状态等）。
典型工作流:
1. 传入 match_id 和 limit 获取关键球员信息
2. 返回主队和客队的关键射手及球员近期状态数据
...
- limit: 返回数据条数限制（默认10条，范围1-50）""",
    ...
    async def _lottery_fetch_players(params: MatchIdWithLimitInput, ctx: Context) -> str:
        return await _lottery_get_players(params.match_id, params.limit, ctx)

    logger.info("滚球盘工具注册完成：lottery_fetch_live_odds")
