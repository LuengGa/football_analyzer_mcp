...
from lottery_mcp.analysis.constants import EMPIRICAL_RETURN_RATES, RETURN_RATE_DEVIATION_THRESHOLD
from lottery_mcp.analysis.calibration.cross_play_validator import CrossPlayValidator
from .output_schemas import (
    MatchPlaysAnalysisOutput,
    AnalyzeWithPipelineOutput,
    PredictWithModelOutput,
    AssessRiskOutput,
)
...
class AnalysisEngine:
    ...
    async def analyze_match(self, match_id: str, lottery_type: str = "竞彩足球",
        ...
        try:
            ...
            # 尝试按玩法获取实证返还率
            empirical_rr = EMPIRICAL_RETURN_RATES.get(lottery_type)
            if empirical_rr:
                return_rate = empirical_rr
    ...
    def _calculate_combined_score_stage2(self, poisson: Dict, elo: Dict) -> float:
        """stage2 综合评分计算（使用动态权重）"""
        poisson_score = poisson.get("win_prob", 0.33) * 100
        elo_score = elo.get("win_prob", 0.33) * 100

        # 使用动态权重计算
        from lottery_mcp.analysis.dynamic_weights import calculate_dynamic_weights
        weights = calculate_dynamic_weights(
            has_elo_data=True,  # stage2有Elo数据
            has_xg_data=False,  # stage2没有xG数据
        )
        
        # 动态权重：泊松和Elo
        poisson_weight = weights.get("poisson", 0.6)
        elo_weight = weights.get("elo", 0.4)
        
        combined = poisson_score * poisson_weight + elo_score * elo_weight
    ...
    def _calculate_combined_score(self, poisson: Dict, elo: Dict, xg: Dict) -> float:
        ...
        from lottery_mcp.analysis.dynamic_weights import calculate_dynamic_weights
        weights = calculate_dynamic_weights(
            has_elo_data=elo.get("home_elo") is not None,
            has_xg_data=xg.get("home_xg") is not None,
        )
        
        # 动态权重
        poisson_weight = weights.get("poisson", 0.4)
        elo_weight = weights.get("elo", 0.3)
        xg_weight = weights.get("xg", 0.3)

        combined = poisson_score * poisson_weight + elo_score * elo_weight + xg_score * xg_weight
        return round(combined, 1)
    ...
    async def analyze_match_plays(self, match_id: str, lottery_type: str = "竞彩足球",
                           handicap: float = 0.0, use_new_pipeline: bool = True) -> Dict[str, Any]:
        """分析比赛的所有五大玩法
        ...
        Args:
            ...
            use_new_pipeline: 是否使用新的统一流水线（默认True）

        Returns:
            五大玩法的分析结果，含协同验证和组合推荐
        """
        try:
            # 首先获取比赛的基础分析结果
            ...
            if use_new_pipeline:
                # 使用新的统一分析流水线
                from lottery_mcp import get_pipeline
                
                pipeline = get_pipeline()
                match_context = {
                    "match_id": match_id,
                    "handicap": handicap,
                    "league": base_analysis.get("match_data", {}).get("league", "default"),
                }
                
                report = pipeline.analyze(match_context, poisson_result, odds)
                ...
                result = {
                    ...
                    "lottery_type": lottery_type,
                    "handicap": handicap,
                    "pipeline_version": report.get("pipeline_version", "2.0"),
                    "plays": {},
                ...
                # 从play_results提取各玩法详情
                play_results = report.get("play_results", {})
                for play_code, play_data in play_results.items():
                    result["plays"][play_code] = {
                        "play_name": self._get_play_name(play_code),
                        "confidence": play_data.get("confidence", 0),
                        "probabilities": play_data.get("probabilities", {}),
                        "recommendations": play_data.get("selections", []),
                        "expected_values": play_data.get("expected_value", 0),
                        "analysis_notes": play_data.get("analysis_notes", []),
                    }
...
async def lottery_get_market_sentiment(params: GetMarketSentimentInput, ctx: Context) -> str:
    ...
    try:
        ...
        # 信号3: 返还率异常
        if had_payout_rate < RETURN_RATE_DEVIATION_THRESHOLD:
            smart_money_signals.append({
...
def register_analysis_tools(mcp):
    ...
    @mcp.tool(
        ...
        description="""[分析] 批量分析当日所有比赛并返回关键指标摘要
...
典型工作流: fetch_today_matches → analyze_all_matches → generate_recommendation""",
    ...
    @mcp.tool(
        ...
        description="""[分析] 批量流水线分析 - 一次分析产出完整数据包

前置条件: 需先调用 lottery_fetch_today_matches 填充数据缓存

典型工作流: fetch_today_matches → analyze_with_pipeline → smart_parlay""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
        ...
        outputSchema=AnalyzeWithPipelineOutput,
    )
    ...
    @mcp.tool(
        ...
        description="""[单场五大玩法分析] 分析单场比赛的五大玩法（胜平负、让球胜平负、比分、总进球、半全场）。""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
        ...
        outputSchema=MatchPlaysAnalysisOutput,
    )
    ...
    @mcp.tool(
        ...
        description="""[分析] 检测比赛风险信号

前置条件: 需先调用 lottery_fetch_today_matches 或 lottery_get_match_context 获取比赛数据

典型工作流: fetch_today_matches → detect_risk_signals → assess_risk → generate_recommendation""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[分析] 对比多个统计模型的预测结果

前置条件: 需先调用 lottery_predict_with_model 获取单模型预测结果

典型工作流: predict_with_model → compare_model_predictions → generate_recommendation""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
...
数据来源：500.com / sporttery.cn。""",
    ...
    @mcp.tool(
        ...
        description="""[发现] ⚠️ 前置条件: 请先调用 `lottery_fetch_today_matches` 获取并缓存今日比赛数据，否则本工具将返回空结果。

典型工作流: get_market_odds(获取赔率,""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[分析] ⚠️ 前置条件: 请先调用 `lottery_fetch_today_matches` 获取并缓存今日比赛数据，否则本工具将返回空结果。

典型工作流: fetch_today_matches(获取match_id,""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[预测] 使用ML模型预测比赛结果

典型工作流: fetch_today_matches → predict_with_model → compare_model_predictions(多模型对比,""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
        ...
        outputSchema=PredictWithModelOutput,
    )
...
典型工作流:""",
    ...
    @mcp.tool(
        ...
        description="""[数据] 获取比赛完整上下文（AI推理工具）
...
典型工作流:""",
    ...
    @mcp.tool(
        ...
        description="""[评估] 多维度风险评估（AI推理工具）

典型工作流: get_match_context → assess_risk → simulate_scenarios(情景模拟)""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[生成] 生成综合投注建议（AI推理工具）

典型工作流: analyze_match → assess_risk → generate_recommendation → validate_scenario(规则验证)""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[分析] 多场比赛对比分析，找出相似模式""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[优化] 资金分配优化（凯利公式+风险平价+最大回撤）""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": False,
            "idempotentHint": True,
