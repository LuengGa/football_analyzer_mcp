...
async def _analyze_single_match(
    ...
    # 修复：添加consensus和kelly到odds
    if match.get("consensus"):
        a.odds["consensus"] = match["consensus"]
    if match.get("kelly"):
        a.odds["kelly"] = match["kelly"]
    # ========== 修复结束 ==========

    # 2. 基本面信息（整合竞彩官网7大资讯 + 国际市场数据）
    fundamentals = {
    ...
    if a.match_id:
        try:
            ...
            # 2.1 历史交锋
            try:
                h2h_data = await mgr.get_result_history(a.match_id, term_limits=10)
                if h2h_data and h2h_data.get("ok"):
                    fundamentals["head_to_head_detail"] = h2h_data.get("data", [])
                    fundamentals["head_to_head"] = f"近{len(fundamentals['head_to_head_detail'])}场交锋记录"
                    logger.debug(f"比赛 {a.match_id}: 获取历史交锋 {len(fundamentals['head_to_head_detail'])} 条")
            except Exception as e:
                logger.debug(f"获取历史交锋失败: {e}")

            # 2.2 积分榜
            try:
                tables_data = await mgr.get_match_tables(a.match_id)
                if tables_data and tables_data.get("ok"):
                    fundamentals["league_tables"] = tables_data.get("data", {})
                    # 提取排名信息
                    tables = fundamentals["league_tables"]
                    if isinstance(tables, dict):
                        home_table = tables.get("home_team_table", {})
                        away_table = tables.get("away_team_table", {})
                        if home_table:
                            fundamentals["home_rank"] = home_table.get("rank", fundamentals["home_rank"])
                            fundamentals["home_win_rate"] = home_table.get("win_rate", fundamentals["home_win_rate"])
                        if away_table:
                            fundamentals["away_rank"] = away_table.get("rank", fundamentals["away_rank"])
                            fundamentals["away_win_rate"] = away_table.get("win_rate", fundamentals["away_win_rate"])
                    logger.debug(f"比赛 {a.match_id}: 获取积分榜")
            except Exception as e:
                logger.debug(f"获取积分榜失败: {e}")

            # 2.3 比赛近况
            try:
                form_data = await mgr.get_match_recent_form(a.match_id, term_limits=5)
                if form_data and form_data.get("ok"):
                    fundamentals["recent_form_detail"] = form_data.get("data", {})
                    fundamentals["recent_form"] = "近期战绩已获取"
                    logger.debug(f"比赛 {a.match_id}: 获取比赛近况")
            except Exception as e:
                logger.debug(f"获取比赛近况失败: {e}")

            # 2.4 特征分析
            try:
                feature_data = await mgr.get_match_feature(a.match_id)
                if feature_data and feature_data.get("ok"):
                    fundamentals["match_features"] = feature_data.get("data", {})
                    logger.debug(f"比赛 {a.match_id}: 获取特征分析")
            except Exception as e:
                logger.debug(f"获取特征分析失败: {e}")

            # 2.5 射手信息
            try:
                players_data = await mgr.get_match_players(a.match_id)
                if players_data and players_data.get("ok"):
                    fundamentals["top_scorers"] = players_data.get("data", {})
                    logger.debug(f"比赛 {a.match_id}: 获取射手信息")
            except Exception as e:
                logger.debug(f"获取射手信息失败: {e}")

            # 2.6 未来赛事
            try:
                future_data = await mgr.get_future_matches(a.match_id)
                if future_data and future_data.get("ok"):
                    fundamentals["future_matches"] = future_data.get("data", [])
                    logger.debug(f"比赛 {a.match_id}: 获取未来赛事")
            except Exception as e:
                logger.debug(f"获取未来赛事失败: {e}")

        except Exception as e:
            logger.warning(f"调用竞彩官网资讯接口失败: {e}")
    # ========== 新增结束 ==========
    ...
    # 4. 五大玩法概率（原始分析）
    # ========== 新增：尝试从缓存获取 ==========
    cache_key = f"play_analysis:{a.match_id}:{a.handicap}"
    cached_plays = None
    try:
        from lottery_mcp.data.cache import get_cache
        cache = get_cache()
        cached_plays = cache.get(cache_key)
    except Exception as e:
        logger.debug(f"赔率解析失败: {e}")
        pass
    ...
    # 使用缓存或重新计算
    if cached_plays:
        a.plays = cached_plays
        logger.debug(f"比赛 {a.match_id}: 使用缓存的玩法分析结果")
    else:
        ...
        # 存入缓存 (TTL=30分钟，比赛数据会变化)
        try:
            cache.set(cache_key, a.plays, ttl=1800, data_source="play_analysis")
        except Exception as e:
            logger.debug(f"数据提取失败: {e}")
            pass
    ...
    try:
        ...
        synergy_analyzer = get_synergy_analyzer()
        
        # 准备验证数据
        validation_data = {}
        for play_type in ["SPF", "RQSPF", "BF", "ZJQ", "BQC"]:
            if play_type in a.plays:
                validation_data[play_type] = a.plays[play_type].get("probabilities", {})
        validation_data["handicap"] = a.handicap
        
        # 执行验证
        synergy_result = synergy_analyzer.validate_all_plays_consistency(validation_data)
        a.plays["_synergy_validation"] = synergy_result
        
        # 应用置信度提升
        if synergy_result.get("overall_consistent"):
            confidence_boost = synergy_result.get("total_confidence_boost", 0)
            for play_type in a.plays:
                if isinstance(a.plays[play_type], dict) and "confidence" in a.plays[play_type]:
                    original_confidence = a.plays[play_type]["confidence"]
                    if isinstance(original_confidence, (int, float)):
                        a.plays[play_type]["confidence"] = min(1.0, original_confidence + confidence_boost)
        
        # 发现协同机会
        opportunities = synergy_analyzer.find_synergy_opportunities(validation_data)
        a.plays["_synergy_opportunities"] = opportunities
        
        logger.debug(f"比赛 {a.match_id}: 协同验证完成，一致性={synergy_result.get('overall_consistent')}")
    except Exception as e:
        logger.debug(f"协同验证跳过: {e}")
    # ========== 新增结束 ==========
