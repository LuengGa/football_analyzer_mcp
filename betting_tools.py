...
def register_betting_tools(mcp):
    ...
    @mcp.tool(
        ...
        description="""[数据] ⚠️ 前置条件: 请先调用 `lottery_fetch_today_matches` 获取并缓存今日比赛数据，否则本工具将返回空结果。

前置条件: 请先调用 lottery_fetch_today_matches 填充数据缓存。

典型工作流: lottery_fetch_today_matches(获取比赛) → lottery_analyze_all_matches(批量分析) → lottery_get_daily_recommendations(获取推荐)""",
        annotations={
            "readOnlyHint": True,
        ...
        outputSchema=DailyRecommendationsOutput,
    )
    ...
    @mcp.tool(
        ...
        description="""[生成] ⚠️ 前置条件: 请先调用 `lottery_fetch_today_matches` 获取并缓存今日比赛数据，否则本工具将返回空结果。

典型工作流: analyze_match(分析) → generate_betting_slips(生成) → validate_parlay(验证)""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": True,   # 生成投注单文件
            "idempotentHint": True,
            "openWorldHint": False,
        },
        outputSchema=BettingSlipsOutput,
    )
    ...
    @mcp.tool(
        ...
        description="""[生成] 生成凯利公式投注单

典型工作流: predict_with_model(获取概率) → generate_kelly_slips(计算最优投注) → record_bet(记录)""",
        annotations={
            "readOnlyHint": True,
            "destructiveHint": True,   # 生成投注单文件
            "idempotentHint": True,
    ...
    @mcp.tool(
        ...
        description="""[数据] 获取投注统计（内存版，仅当前会话）
返回当前会话中通过 lottery_track_bet 记录的投注统计。
注意：如需完整的持久化投注统计（跨会话），请使用 lottery_get_bet_statistics。""",
        ...
        outputSchema=BettingStatsOutput,
    )
